...
func (b *HuntTask) StartHunt() {
	...
	for i := 0; i < len(b.GC.Configs.HuntConfig.TargetItems); i++ {
		...
		log.Infof("当前%s数量 %d", b.GC.Configs.HuntConfig.TargetItems[i])
	...
	go func() {
		for {
			...
			case <-ctx.Done():
				...
				return
			case <-ticker.C:
				if targetId != 0 && b.GC.AtkStat.GetCurrentTargetId() == targetId && time.Since(lastPosUpdate) > time.Second*3 {
					log.Infof("卡住了")
			...
			case <-ticker2.C:
				for i := 0; i < len(b.GC.Configs.HuntConfig.TargetItems); i++ {
					curCount := b.getItemCount(b.GC.Configs.HuntConfig.TargetItems[i])
					log.Infof("当前%s数量 %d, 打了 %d", b.GC.Configs.HuntConfig.TargetItems[i], curCount, curCount-ItemCount[i])
				}
			}
	...
	for {
		...
		if b.GC.Role.GetMapId() != MapID {
			if MapID == gameTypes.MapId_LhzDun03.Uint32() {
				...
				b.GC.ChangeMap(gameTypes.MapId_LhzDun01.Uint32())
				for i := 0; i < len(b.GC.Configs.HuntConfig.TargetItems); i++ {
					b.GC.MoveChartWait(Pos_03[i])
				}
			} else {
				b.GC.GoToMap(MapID)
				b.GC.ChangeMap(MapID)
			}

			b.EnableGodMode()

		} else {
			if b.GC.Configs.HuntConfig.TimerFly > 0 && time.Since(lastFlyTime) > time.Second*time.Duration(b.GC.Configs.HuntConfig.TimerFly) {
				b.useFlyWing()
